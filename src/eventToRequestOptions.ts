...
const getValuesFromStringAndMultiString = (
  stringMap: StringMap<string> | null | undefined,
  multiStringMap: StringMap<string[]> | null | undefined,
  lcKeys = true
): StringMap<string> => {
...
const eventToRequestOptions = (
  event: APIGatewayEvent,
  ctx?: LambdaContext
): InjectOptions => {
  let remoteAddress: string | undefined = undefined
  let ssl = false
  const queryStringParams = getValuesFromStringAndMultiString(
    event.queryStringParameters,
    event.multiValueQueryStringParameters,
    false
  )
  const headers = getValuesFromStringAndMultiString(
    event.headers,
