...
import {
  inProcessResponseToLambdaResponse,
  errorResponse,
} from "../src/response"
...
describe("inProcessResponseToLambdaResponse", () => {
  ...
  const generateMockResponse = (
    opts: GenerateMockResponseOptions = {}
  ): Response =>
    ({
  ...
  it("returns 200", () => {
    const res = inProcessResponseToLambdaResponse(
  ...
  describe("when the body is not utf8", () => {
    it("has base64 body", () => {
      const res = inProcessResponseToLambdaResponse(
        generateMockResponse({ contentType: "not-text/plain" }),
        true,
        false
      )
  ...
  describe("UTF8 content", () => {
    it("has utf8 body if content type is set to text and body is utf8", () => {
      const res = inProcessResponseToLambdaResponse(
        generateMockResponse({ contentType: "text/whatever" }),
        false,
        false
      )
      expect(res.isBase64Encoded).toEqual(false)
